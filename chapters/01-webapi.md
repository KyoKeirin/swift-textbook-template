...
1. **質問：**
   Swift言語の「Codable」って何？
   
   **得られた理解：**
   Swiftのデータと JSON を相互変換する仕組みである。
2. **質問：**
   Swift言語の「NavigationStack」って何？
   
   **得られた理解：**
   SwiftUIで画面遷移を管理する仕組みである。
3. **質問：**
   modifierの順番次第で、効果も違うの？
   
   **得られた理解：**
   modifier は包み込みなので、順番が変わると見た目や動作も変わる。
## この章のまとめ
